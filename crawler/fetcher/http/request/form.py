...
def _urlencode(seq, enc):
    values = [(to_bytes(k, enc), to_bytes(v, enc))
              for k, vs in seq for v in (vs if is_listlike(vs) else [vs])]
...
def _get_inputs(form, formdata, dont_click, clickdata, response):
    ...
    if not dont_click:
        ...
        if clickable and clickable[0] not in formdata and not clickable[
            0
        ] is None:
